...
public:
    ...
    public:
         iterator(std::shared_ptr<atomic_list::list_node<T>> node,std::atomic<int> *reader_counter, Atomic_Blist* parent ) : _node(node) {
             parent_=parent;
    ...
    public:
         riterator(std::shared_ptr<atomic_list::list_node<T>> node,std::atomic<int> *reader_counter,Atomic_Blist* parent) : _node(node) {
             parent_=parent;
            _reader_counter=reader_counter;
            _reader_counter->fetch_add(1);
...
//            std::cout<<"no of active readers= "<<tmp<<std::endl;
    ...
    if(someone_is_writing){ no_of_waiting_readers.fetch_add(1); wait(mozna_czytac); no_of_waiting_readers.fetch_sub(1); }
